...
import sys
from custom_ds_builder.conversion_utils import MultiThreadedDatasetBuilder
...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_example(episode_path, demo_id):
        ...
        with h5py.File(episode_path, "r") as F:
            ...
            images = F['data'][f"demo_{demo_id}"]["obs"]["image"][()]
            wrist_images = F['data'][f"demo_{demo_id}"]["obs"]["wrist"][()]
        ...
        for i in range(actions.shape[0]):
            episode.append({
                'observation': {
                    ...
                    'state': np.asarray(joint_states[i], dtype=np.float32),
...
class CustomerTest(MultiThreadedDatasetBuilder):
    """DatasetBuilder for example dataset."""
    TAG = "2.0.0"

    VERSION = tfds.core.Version(TAG)
    RELEASE_NOTES = {
      TAG: 'Initial release.',
    }
    ...
    def _split_paths(self):
        ...
        return {
            "train": glob.glob("/home/phibot/Work/robot/dataset/rlds_custom_dataset_builder/dataset/archive_ds/*.hdf5"),
        }
